...
import { LOCS, CATS, ENVS, M, ML } from '../data/constants.js';
    ...
    run.categories.forEach(cat => {
      cat.tests.forEach((test, idx) => {
        ...
        if (test.status === 'passed' || test.status === 'failed') {
          seenTests.add(key);
          const catDef = CATS.find(c => c.id === cat.id);
...
export default function Execute() {
  ...
  const { runs, addRun, updateTest, updateRun, pushEvent, setActiveId, setView, selEnvVal, setSelEnv, aiCache, setCacheItem } = store;

  const [selectedLoc, setSelectedLoc] = useState(Object.keys(LOCS)[0]);
  const [selectedCluster, setSelectedCluster] = useState(LOCS[Object.keys(LOCS)[0]].clusters[0]);
  const [selectedScope, setSelectedScope] = useState(CATS.map(c => c.id));
  ...
  const locOptions = Object.entries(LOCS);
  const clusterOptions = LOCS[selectedLoc]?.clusters || [];

  const handleLocChange = (e) => {
    setSelectedLoc(e.target.value);
    setSelectedCluster(LOCS[e.target.value].clusters[0]);
  };
  ...
  const selectAll = () => setSelectedScope(CATS.map(c => c.id));
  const selectCritical = () => setSelectedScope(CATS.filter(c => c.critical).map(c => c.id));
  const clearAll = () => setSelectedScope([]);

  const selectedCats = CATS.filter(c => selectedScope.includes(c.id));
  ...
  const runPreflight = async () => {
    ...
    let text = '';
    await llm(
      [{ role: 'user', content: `Pre-flight check for OpenShift cluster ${selectedCluster} (${LOCS[selectedLoc].label}), environment: ${selEnvVal}. Test categories: ${selectedCats.map(c => c.name).join(', ')}. Total tests: ${totalTests}. List key risks and readiness checks needed. Be concise.` }],
      M.preflight,
  ...
  return (
    <div className="execute-layout">
      <div className="execute-main">
        ...
        <div className="card">
          <div className="card-header">
            <div className="card-title">🎯 Test Scope ({selectedScope.length}/{CATS.length} categories · {totalTests} tests)</div>
        ...
        <div className="card">
          ...
          <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
              ...
              <span className="font-bold">{LOCS[selectedLoc]?.label}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
